...
local function GetInfoTooltip()
    return MS:GetTooltipFrame("Info")
end

local function FetchPlayerLockouts(tooltip)
    ...
    if #DungeonLockouts > 0 or #RaidLockouts > 0 and (MS.db.global.Tooltip.Time.Date or MS.db.global.Tooltip.Time.AlternateTime) then tooltip:AddLine(" ", 1, 1, 1, 1) end
    if #DungeonLockouts > 0 then
        tooltip:AddLine("|c" .. AccentColour .. "Dungeon|r |cFFFFFFFFLockouts|r", 1, 1, 1, 1)
        for _, Lockout in pairs(DungeonLockouts) do
            ...
            tooltip:AddDoubleLine(DungeonDisplayString, DungeonReset, 1, 1, 1, 1, 1, 1)
        end
    end
    if #RaidLockouts > 0 then
        ...
        tooltip:AddLine("|c" .. AccentColour .. "Raid|r |cFFFFFFFFLockouts|r", 1, 1, 1, 1)
        for _, Lockout in pairs(RaidLockouts) do
            ...
            tooltip:AddDoubleLine(RaidDisplayString, RaidReset, 1, 1, 1, 1, 1, 1)
        end
    end
end
...
local function FetchVaultOptions(tooltip)
    local GeneralDB = MS.db.global.General
    local AccentColour = GeneralDB.ClassColour and string.format("FF%02x%02x%02x", MS.CLASS_COLOUR[1], MS.CLASS_COLOUR[2], MS.CLASS_COLOUR[3]) or string.format("FF%02x%02x%02x", GeneralDB.AccentColour[1], GeneralDB.AccentColour[2], GeneralDB.AccentColour[3])
    ...
    local function FetchRaidData()
        ...
        for i = 1, 3 do
            if not RaidRuns[i] then break end
        ...
        if #RaidsCompleted > 0 then
            tooltip:AddLine("|c" .. AccentColour .. "Raid|r |cFFFFFFFFGreat Vault|r", 1, 1, 1, 1)
            for _, Raid in pairs(RaidsCompleted) do
                tooltip:AddLine(Raid, 1, 1, 1)
            end
        end
    end
    ...
    local function FetchMythicPlusData()
        ...
        for i = 1, 3 do
            if not MythicPlusRuns[i] then break end
        ...
        if #MythicPlusRunsCompleted > 0 then
            ...
            tooltip:AddLine("|c" .. AccentColour .. "Mythic+|r |cFFFFFFFFGreat Vault|r", 1, 1, 1, 1)
            for _, Key in pairs(MythicPlusRunsCompleted) do
                tooltip:AddLine(Key, 1, 1, 1)
            end
        end
    end
    ...
    local function FetchWorldData()
        ...
        for i = 1, 3 do
            if not WorldRuns[i] then break end
        ...
        if #WorldRunsCompleted > 0 then
            if #RaidsCompleted > 0 or #MythicPlusRunsCompleted > 0 then tooltip:AddLine(" ", 1, 1, 1, 1) end
            tooltip:AddLine("|c" .. AccentColour .. "World|r |cFFFFFFFFGreat Vault|r", 1, 1, 1, 1)
            for _, Delve in pairs(WorldRunsCompleted) do
                tooltip:AddLine(Delve, 1, 1, 1)
            end
        end
    end
    FetchRaidData()
    FetchMythicPlusData()
    FetchWorldData()
    if #RaidsCompleted > 0 or #MythicPlusRunsCompleted > 0 or #WorldRunsCompleted > 0 then tooltip:AddLine(" ", 1, 1, 1, 1) end
end

local function CreateTimeTooltip(displayDate, displayLockouts, displayAlternateTime)
    local GeneralDB = MS.db.global.General
    local AccentColour = GeneralDB.ClassColour and string.format("FF%02x%02x%02x", MS.CLASS_COLOUR[1], MS.CLASS_COLOUR[2], MS.CLASS_COLOUR[3]) or string.format("FF%02x%02x%02x", GeneralDB.AccentColour[1], GeneralDB.AccentColour[2], GeneralDB.AccentColour[3])
    local tooltip = GetInfoTooltip()
    local minimap = MS:GetMinimapReferenceFrame() or Minimap or UIParent

    tooltip:SetOwner(minimap, "ANCHOR_NONE")
    tooltip:ClearAllPoints()
    tooltip:SetPoint("TOPRIGHT", minimap, "BOTTOMRIGHT", 0, -1)
    tooltip:ClearLines()

    if displayDate and not displayAlternateTime then
        local dateString = date(FetchDateString())
        tooltip:AddLine(dateString, 1, 1, 1)
    elseif displayDate and displayAlternateTime then
        ...
        tooltip:AddDoubleLine(dateString, alternateTimeString, 1, 1, 1, 1, 1, 1)
    elseif not displayDate and displayAlternateTime then
        local alternateTimeString = string.format("|c%s%s|r Time: %s", AccentColour, MS.db.global.Time.TimeZone == "Local" and "Server" or "Local", FetchAlternateTime())
        tooltip:AddLine(alternateTimeString, 1, 1, 1)
    end

    if displayLockouts then FetchPlayerLockouts(tooltip) end

    if displayLockouts or displayDate or displayAlternateTime then tooltip:AddLine(" ", 1, 1, 1, 1) end
    tooltip:AddDoubleLine(MS.LEFT_CLICK_BUTTON .. "|c" .. AccentColour .. "Left-Click|r", "Open Calendar", 1, 1, 1, 1, 1, 1)
...
local function CreateSystemStatsTooltip(displayVaultOptions)
    ...
    if displayVaultOptions then
        FetchVaultOptions(tooltip)
    end

    tooltip:AddDoubleLine(MS.RIGHT_CLICK_BUTTON .. "|c" .. AccentColour .. "Right-Click|r", "Open Configuration", 1, 1, 1, 1, 1, 1)
    tooltip:AddDoubleLine(MS.MIDDLE_CLICK_BUTTON .. "|c" .. AccentColour .. "Middle-Click|r", "Reload UI", 1, 1, 1, 1, 1, 1)
...
function MS:AssignTooltipScripts()
    ...
    MS.TimeFrame:SetScript("OnLeave", function(self) MS:HideTooltipFrame("Info") end)
    ...
    MS.SystemStatsFrame:SetScript("OnLeave", function(self) MS:HideTooltipFrame("Info") end)
end
