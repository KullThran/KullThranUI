...
function MS:CreateInstanceDifficulty()
    ...
    MS:ApplyDetachedMinimapLayout(InstanceDifficultyFrame, DB.Layout)
    ...
    InstanceDifficultyFrame.Text:SetFont(MS.ResolveFont(GeneralDB.Font), DB.Layout[5], GeneralDB.FontFlag)
    ...
    if DB.Enable then
        ...
        InstanceDifficultyFrame:SetScript("OnEvent", function(self, event, ...)
            ...
            MS:ApplyDetachedMinimapLayout(self, DB.Layout)
        end)
    else
...
function MS:UpdateInstanceDifficulty()
    ...
    if MS.InstanceDifficultyFrame then
        ...
        MS:ApplyDetachedMinimapLayout(MS.InstanceDifficultyFrame, DB.Layout)
        MS.InstanceDifficultyFrame:SetFrameStrata(GeneralDB.FrameStrata)
        MS.InstanceDifficultyFrame.Text:SetFont(MS.ResolveFont(GeneralDB.Font), DB.Layout[5], GeneralDB.FontFlag)
