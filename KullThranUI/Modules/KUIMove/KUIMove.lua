...
local function CanIterate(value)
...
local damageMeterFrameNameSet = {};
for _, frameName in ipairs(damageMeterFrameNames) do
    damageMeterFrameNameSet[frameName] = true;
end

local function IsSensitiveBlizzardFrame(frameName, frame)
    if not IsSecret(frameName) and frameName and damageMeterFrameNameSet[frameName] then
        return true
    end
    ...
    while cur and depth < 8 do
        if cur.GetName then
            local ok, curName = pcall(cur.GetName, cur)
            if ok and not IsSecret(curName) and curName and damageMeterFrameNameSet[curName] then
                return true
            end
        end
...
local function DisableDamageMeterFrames(self)
    self.DB.disabledFrames = self.DB.disabledFrames or {};
    self.DB.enabledFrames = self.DB.enabledFrames or {};

    for _, addOnName in ipairs({ INTERNAL_ADDON_NAME, "Blizzard_DamageMeter" }) do
        ...
        for _, frameName in ipairs(damageMeterFrameNames) do
            ...
            self.DB.enabledFrames[addOnName][frameName] = nil;
            if self.DB.points then
                self.DB.points[frameName] = nil;
            end
            if self.DB.scales then
                self.DB.scales[frameName] = nil;
            end

            if self.Frames and self.Frames[addOnName] then
                self.Frames[addOnName][frameName] = nil;
            end
            if self.FrameRegistry and self.FrameRegistry[addOnName] then
                self.FrameRegistry[addOnName][frameName] = nil;
            end

            local liveFrame = _G[frameName]
            if liveFrame and self.FrameData and self.FrameData[liveFrame] and self.FrameData[liveFrame].storage then
                self:MakeFrameUnmovable(liveFrame, self.FrameData[liveFrame])
                self.FrameData[liveFrame].storage.disabled = true;
            end
        end
    end
end

local questMapSensitiveFrameNames = {
    "QuestMapFrame",
    "QuestMapFrame.DetailsFrame.RewardsFrame",
    "QuestMapFrame.DetailsFrame.ScrollFrame",
};

local function DisableQuestMapFrames(self)
    self.DB.disabledFrames = self.DB.disabledFrames or {};
    self.DB.enabledFrames = self.DB.enabledFrames or {};

    local addOnName = INTERNAL_ADDON_NAME;
    self.DB.disabledFrames[addOnName] = self.DB.disabledFrames[addOnName] or {};
    self.DB.enabledFrames[addOnName] = self.DB.enabledFrames[addOnName] or {};

    for _, frameName in ipairs(questMapSensitiveFrameNames) do
        self.DB.disabledFrames[addOnName][frameName] = true;
        self.DB.enabledFrames[addOnName][frameName] = nil;
        ...
        local liveFrame = self.GetFrameFromName and self:GetFrameFromName(addOnName, frameName) or nil;
        if liveFrame and self.FrameData and self.FrameData[liveFrame] then
            self:UnprocessFrame(addOnName, frameName);
        end
        if self.FrameRegistry and self.FrameRegistry[addOnName] and self.FrameRegistry[addOnName][frameName] then
            self.FrameRegistry[addOnName][frameName] = nil;
        end
    end
end
        ...
        for key, value in pairs(frameData) do
            if key == "SubFrames" then
                ...
                for subFrameName, subFrameData in pairs(value) do
                    if not self:ValidateFrame(subFrameName, subFrameData, true) then
    ...
    function DoOnMouseUp(frame, button, moveHandle)
        ...
        if frameData.storage.detached or not parentReturnValue then
            ...
            elseif button == "RightButton" then
                ...
                if SafeBooleanCall(IsAltKeyDown) and frameData.storage.detached then
                ...
                if SafeBooleanCall(IsControlKeyDown) or fullReset then
                    returnValue = SetFrameScale(frame, 1) or returnValue;
                end

                if SafeBooleanCall(IsShiftKeyDown) or fullReset then
...
do
    ...
    function KUIMove:InitMouseWheelCaptureFrame()
        ...
        local wheelCaptureAccum = 0
        local wheelCaptureInterval = 0.08
        captureFrame:SetScript("OnUpdate", function(_, elapsed)
            wheelCaptureAccum = wheelCaptureAccum + elapsed
            if wheelCaptureAccum >= wheelCaptureInterval then
                wheelCaptureAccum = 0
                self:CheckMouseWheelCapture()
            end
        end);
    ...
    function KUIMove:CheckMouseWheelCapture()
        ...
        local controlDown = IsControlKeyDown();
        local mouseFoci = GetMouseFoci();
        if IsSecret(controlDown) or not controlDown or not next(self.CurrentMouseoverFrames) or not CanIterate(mouseFoci) or not next(mouseFoci) then
            -- Keep mouse wheel disabled whenever the input state is private or
            -- no registered movable frame is currently under the cursor.
            return;
        end
        ...
        for _, frame in ipairs(mouseFoci) do
            ...
            if not shouldHandleMouseWheel then
                local forbidden = frame.IsForbidden and frame:IsForbidden();
                if IsSecret(forbidden) or forbidden then return; end

                local frameName;
                if frame.GetName then frameName = frame:GetName(); end
                if IsSecret(frameName) or IsSensitiveBlizzardFrame(frameName, frame) then return; end

                if not self.MoveHandles[frame] then
                    local wheelEnabled = frame.IsMouseWheelEnabled and frame:IsMouseWheelEnabled();
                    if IsSecret(wheelEnabled) or wheelEnabled then return; end

                    local clickEnabled = frame.IsMouseClickEnabled and frame:IsMouseClickEnabled();
                    if IsSecret(clickEnabled) or clickEnabled then return; end
                end
            end
...
do
    ...
    function KUIMove:ProcessFrame(addOnName, frameName, frameData, frameParent, retriedAfterNotFound)
        ...
        if IsSensitiveBlizzardFrame(frameName, frame) then
            self:UnregisterFrame(addOnName, frameName, true);
            return false;
        end
...
do
    ...
    function KUIMove:OnInitialize()
        ...
        -- taint paths in combat/PvP.
        DisableDamageMeterFrames(self);
        -- Modern retail QuestMap rows carry secret numeric payloads. Leaving the
        -- quest panel fully Blizzard-owned avoids taint in list refresh/update paths.
        DisableQuestMapFrames(self);
        ...
        self:ProcessFrames(INTERNAL_ADDON_NAME);
        self:ApplyAddOnSpecificFixes(INTERNAL_ADDON_NAME);
        ...
        for addOnName in pairs(self.Frames) do
            if addOnName ~= INTERNAL_ADDON_NAME and IsAddOnLoaded(addOnName) then
    ...
    function KUIMove:ApplyAddOnSpecificFixes(addOnName)
        ...
        if addOnName == INTERNAL_ADDON_NAME then
