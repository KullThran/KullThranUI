...
local function UpdateColor(self, event, unit)
	...
	elseif(element.colorClass and (UnitIsPlayer(unit) or UnitInPartyIsAI(unit)))
		...
		if issecretvalue and issecretvalue(rawClass) then
			-- Hostile and identity-restricted players can expose a secret class
			-- token. Blizzard's class-color API accepts it without Lua using it
			-- as a table key, which preserves class coloring without taint errors.
			if C_ClassColor and C_ClassColor.GetClassColor then
				color = C_ClassColor.GetClassColor(rawClass)
			end
		else
