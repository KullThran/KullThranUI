...
local function SafeGetAuraSlots(unit, filter)
    local function PackSlots(ok, ...)
        if not ok then return { n = 0 } end
        return { n = select("#", ...), ... }
    end
    return PackSlots(pcall(C_UnitAuras.GetAuraSlots, unit, filter, 40))
end

local function CanIterateAuraUpdateList(value)
...
local function CanUseIncrementalAuraUpdate(updateInfo)
	if(type(updateInfo) ~= 'table') then return false end
	local ok, isFullUpdate, added, updated, removed = pcall(function()
		return updateInfo.isFullUpdate, updateInfo.addedAuras,
			updateInfo.updatedAuraInstanceIDs, updateInfo.removedAuraInstanceIDs
	end)
	if(not ok) then return false end
	if(issecretvalue) then
		local secretOK, secret = pcall(issecretvalue, isFullUpdate)
		if(not secretOK or secret) then return false end
	end
	if(canaccessvalue) then
		local accessOK, accessible = pcall(canaccessvalue, isFullUpdate)
		if(not accessOK or not accessible) then return false end
	end
	if(isFullUpdate) then return false end
	return CanIterateAuraUpdateList(added)
		and CanIterateAuraUpdateList(updated)
		and CanIterateAuraUpdateList(removed)
end
...
local function CreateButton(element, index)
	...
	overlay.SetVertexColor = function(self, r, g, b, a)
		a = a or 1
		local dr, dg, db = r * 0.15, g * 0.15, b * 0.15
		top:SetVertexColor(r, g, b, a)
		bottom:SetVertexColor(dr, dg, db, a)
		if left.SetGradient then
			left:SetGradient("VERTICAL", CreateColor(dr, dg, db, a), CreateColor(r, g, b, a))
			right:SetGradient("VERTICAL", CreateColor(dr, dg, db, a), CreateColor(r, g, b, a))
		elseif left.SetGradientAlpha then
			left:SetGradientAlpha("VERTICAL", dr, dg, db, a, r, g, b, a)
			right:SetGradientAlpha("VERTICAL", dr, dg, db, a, r, g, b, a)
		end
	end
...
local function processData(element, unit, data, filter)
	...
	if type(data.auraInstanceID) == 'number' then
		local ok, filtered = pcall(C_UnitAuras.IsAuraFilteredOutByInstanceID, unit, data.auraInstanceID, filter .. '|PLAYER')
		if ok then isPlayerAura = not filtered end
	end
	data.isPlayerAura = isPlayerAura
...
local function UpdateAuras(self, event, unit, updateInfo)
	...
	-- Never consume UNIT_AURA's incremental payload. In PvP Blizzard can make
	-- its nested tables inaccessible after a successful `next` probe, so a
	-- check followed by iteration is inherently racy in tainted execution.
	-- The slot APIs below return positional values copied into our own table.
	...
	if(auras) then
		...
		* isFullUpdate - indicates whether the element is performing a full update (boolean)
		...
		if(isFullUpdate) then
			...
			for i = 2, slots.n do -- #1 return is continuationToken, we don't care about it
