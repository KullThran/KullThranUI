...
local function clearAndHide(element)
	element.delay = 0
	element.holdTime = 0
...
local function CastStop(self, event, unit, _, _, ...)
	...
	if(interruptedBy and element.holdTime and element.holdTime > 0) then
		resetAttributes(element)
	else
		clearAndHide(element)
	end
...
local function onUpdate(self, elapsed)
	if(self.casting or self.channeling or self.empowering) then
		...
		if(unit) then
			local activeCast = UnitCastingInfo(unit)
			local activeChannel = UnitChannelInfo(unit)
			if(not activeCast and not activeChannel) then
				clearAndHide(self)
				return
			end
		end
