...
local tagStrings = {
	...
	['leader'] = [[function(u)
		local value = UnitIsGroupLeader(u)
		if(not (issecretvalue and issecretvalue(value)) and value == true) then
			return 'L'
		end
	end]],
	...
	['raidcolor'] = [[function(u)
		...
		if _G.KullThranUI and _G.KullThranUI.SafeUnitClass then
			_, class = _G.KullThranUI.SafeUnitClass(u)
		else
			_, class = UnitClass(u)
		end
		if not (issecretvalue and issecretvalue(class)) and class then
			local color = _COLORS.class[class]
			if color then return color:GenerateHexColorMarkup() end
		else
			local id = u:match('arena(%d)$')
			if(id) then
				local now = GetTime()
				_ktArenaSpecTime = _ktArenaSpecTime or {}
				_ktArenaSpecCache = _ktArenaSpecCache or {}
				if not _ktArenaSpecTime[id] or now - _ktArenaSpecTime[id] > 2.0 then
					_ktArenaSpecTime[id] = now
					local specID = GetArenaOpponentSpec(tonumber(id))
					if(specID and specID > 0) then
						local _, _, _, _, _, class = GetSpecializationInfoByID(specID)
						if class and _COLORS.class[class] then
							_ktArenaSpecCache[id] = _COLORS.class[class]:GenerateHexColorMarkup()
						end
					end
				end
				return _ktArenaSpecCache[id]
			end
		end
	end]],
