...
local function Update(self, event)
	...
	* self - the AssistantIndicator element
	...
	local assistant = UnitIsGroupAssistant(unit)
	local leader = UnitIsGroupLeader(unit)
	local inRaidReadable = not (issecretvalue and issecretvalue(inRaid))
	local assistantReadable = not (issecretvalue and issecretvalue(assistant))
	local leaderReadable = not (issecretvalue and issecretvalue(leader))
	local isAssistant = inRaidReadable and assistantReadable and leaderReadable
		and inRaid == true and assistant == true and leader ~= true
	if(isAssistant) then
		element:Show()
	else
		element:Hide()
