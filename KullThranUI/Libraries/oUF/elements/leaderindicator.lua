...
local function Update(self, event)
	...
	* self - the LeaderIndicator element
	...
	-- Check secrecy before evaluating the boolean itself. Secret booleans may
	-- not participate in conditional expressions.
	local leaderReadable = not (issecretvalue and issecretvalue(isLeader))
	if leaderReadable and isLeader == true then
	...
	* isInLFGInstance - indicates whether the current party is subject to LFG restrictions (boolean)
	...
	if(element.PostUpdate) then
		return element:PostUpdate(leaderReadable and isLeader == true or false, isInLFGInstance)
	end
end
