...
local function Update(self, event)
	...
	* self - the RaidRoleIndicator element
	...
	local hasVehicle = UnitHasVehicleUI(unit)
	if(not (issecretvalue and issecretvalue(inRaid))
		and not (issecretvalue and issecretvalue(hasVehicle))
		and inRaid == true and hasVehicle ~= true) then
		local mainTank = GetPartyAssignment('MAINTANK', unit)
		local mainAssist = GetPartyAssignment('MAINASSIST', unit)
		if(not (issecretvalue and issecretvalue(mainTank)) and mainTank == true) then
			isShown = true
			element:SetTexture(MAINTANK_ICON)
			role = 'MAINTANK'
		elseif(not (issecretvalue and issecretvalue(mainAssist)) and mainAssist == true) then
			isShown = true
