if KT_CLIENT_BLOCKED then return end -- pre-12.1 client failsafe (KullThranUI_ClientGate.lua)
-- KullThranUI_AuraKit.lua Shared engine for the 12.1 aura container system. Every
-- KullThranUI module consumes aura displays through this file; modules never call
...
local function ApplyStyleToRegions(button, style)
    ...
    if d.borderHost then
        ...
        if PP and b then
            if b.texture and KullThranUI.ApplyBorderStyle then
                ...
                KullThranUI.ApplySecretSafeBorderStyle(d.borderHost, d, b.size or 1,
    ...
    if d.dispelBorder then
        -- Player aura bars may use non-square icon masks; allow matching ring art.
        d.dispelBorder:SetTexture(style.dispelTexture
        ...
        -- Registration normally follows the static border. A specialized style
        -- may opt into an independent engine-tinted ring; zero thickness disables it.
        local hasBorder = style.border or style.dispelBorderIndependent
        local want = (style.dispelBorder and hasBorder
...
function AK.MakeInitializer(styleKey, extra)
    return function(button)
        ...
        -- button itself keeps its mouse for tooltips. (KT_Nameplates_AuraContainers
        ...
        -- clicks to swap target" report involved a clickable plate aura icon with
...
restyler:SetScript("OnUpdate", function(self)
    ...
    local profileStarted = profiler and profiler:Begin("core.aurakit.restyle")
    ...
    while budget > 0 do
        if not restyleWork then
            ...
            if not key then
                self:Hide()
                if profileStarted then profiler:End("core.aurakit.restyle", profileStarted) end
                return
            end
...
-- zoning) plus forced-restriction test CVars; on each edge, re-probe and drain
...
function AK.CreateContainerShell(parent, spec)
    -- Combat creation is legal since 68914 (PTR-7 notes; field test passed
...
local DISPEL_SLOT_DEFS = {
    { key = 'magic',   token = 'Magic',   layer = 5, colorKey = 'dispelColorMagic',   r = 0.349, g = 0.475, b = 1.000 },
    { key = 'curse',   token = 'Curse',   layer = 4, colorKey = 'dispelColorCurse',   r = 0.636, g = 0.000, b = 0.640 },
    { key = 'disease', token = 'Disease', layer = 3, colorKey = 'dispelColorDisease', r = 0.671, g = 0.384, b = 0.098 },
    { key = 'poison',  token = 'Poison',  layer = 2, colorKey = 'dispelColorPoison',  r = 0.000, g = 0.706, b = 0.286 },
    { key = 'bleed',   token = 'Bleed',   layer = 1, colorKey = 'dispelColorBleed',   r = 0.750, g = 0.150, b = 0.150 },
}

local dispelStyleFingerprints = {}

local function DispelColor(options, def)
    local c = options and options.colors and options.colors[def.colorKey]
    return (c and c.r) or def.r, (c and c.g) or def.g, (c and c.b) or def.b
end

local function ApplyDispelSlotStyle(button, data, style)
    local health = style.healthFrame
    if not health then return end

    if not data.ktDispelEdges then
        data.ktDispelEdges = {}
        for i = 1, 4 do
            data.ktDispelEdges[i] = button:CreateTexture(nil, 'OVERLAY', nil, style.layer or 1)
        end
    end

    local r, g, b = style.r or 1, style.g or 1, style.b or 1
    local alpha = tonumber(style.alpha) or 1
    local thickness = math.max(1, math.min(6, tonumber(style.thickness) or 2))
    local gradientAlpha = math.max(0, math.min(1, tonumber(style.gradientAlpha) or math.min(1, alpha * 0.90)))
    local gradientSize = math.max(0.12, math.min(0.60, tonumber(style.gradientSize) or 0.35))
    local edges = data.ktDispelEdges
    local width, height = 1, 1
    if health.GetWidth and health.GetHeight then
        local okW, valueW = pcall(health.GetWidth, health)
        local okH, valueH = pcall(health.GetHeight, health)
        if okW and tonumber(valueW) then width = tonumber(valueW) end
        if okH and tonumber(valueH) then height = tonumber(valueH) end
    end
    local edgeHeight = math.max(thickness + 3, math.floor(height * gradientSize))
    local edgeWidth = math.max(thickness + 3, math.floor(width * gradientSize))

    edges[1]:ClearAllPoints()
    edges[1]:SetPoint('TOPLEFT', health, 'TOPLEFT')
    edges[1]:SetPoint('TOPRIGHT', health, 'TOPRIGHT')
    edges[1]:SetHeight(edgeHeight)

    edges[2]:ClearAllPoints()
    edges[2]:SetPoint('BOTTOMLEFT', health, 'BOTTOMLEFT')
    edges[2]:SetPoint('BOTTOMRIGHT', health, 'BOTTOMRIGHT')
    edges[2]:SetHeight(edgeHeight)

    edges[3]:ClearAllPoints()
    edges[3]:SetPoint('TOPLEFT', health, 'TOPLEFT')
    edges[3]:SetPoint('BOTTOMLEFT', health, 'BOTTOMLEFT')
    edges[3]:SetWidth(edgeWidth)

    edges[4]:ClearAllPoints()
    edges[4]:SetPoint('TOPRIGHT', health, 'TOPRIGHT')
    edges[4]:SetPoint('BOTTOMRIGHT', health, 'BOTTOMRIGHT')
    edges[4]:SetWidth(edgeWidth)

    edges[1]:SetTexture('Interface\\AddOns\\KullThranUI\\Media\\DF_Gradient_V')
    edges[2]:SetTexture('Interface\\AddOns\\KullThranUI\\Media\\DF_Gradient_V_Rev')
    edges[3]:SetTexture('Interface\\AddOns\\KullThranUI\\Media\\DF_Gradient_H')
    edges[4]:SetTexture('Interface\\AddOns\\KullThranUI\\Media\\DF_Gradient_H_Rev')
    for i = 1, 4 do
        edges[i]:SetBlendMode('BLEND')
        edges[i]:SetVertexColor(r, g, b, gradientAlpha)
        edges[i]:Show()
    end
end

function AK.BuildDispelSlotSpecs(prefix)
    local slots = {}
    for i = 1, #DISPEL_SLOT_DEFS do
        local def = DISPEL_SLOT_DEFS[i]
        slots[i] = {
            key = def.key,
            filter = { 'HARMFUL' },
            candidateFilters = { includeDispelTypes = { [def.token] = true } },
            style = prefix .. ':' .. def.key,
        }
    end
    return slots
end

function AK.ConfigureDispelSlotStyles(prefix, healthFrame, options)
    if not (prefix and healthFrame) then return end
    options = options or {}
    local gradientAlpha = options.gradientAlpha or 0.90
    local gradientSize = options.gradientSize or 0.35
    local parts = {
        tostring(options.alpha or 1),
        tostring(options.thickness or 2),
        tostring(gradientAlpha),
        tostring(gradientSize),
    }
    for i = 1, #DISPEL_SLOT_DEFS do
        local def = DISPEL_SLOT_DEFS[i]
        local r, g, b = DispelColor(options, def)
        parts[#parts + 1] = string.format('%.3f,%.3f,%.3f', r, g, b)
    end
    local fingerprint = table.concat(parts, '|')
    if dispelStyleFingerprints[prefix] == fingerprint then return end
    dispelStyleFingerprints[prefix] = fingerprint

    for i = 1, #DISPEL_SLOT_DEFS do
        local def = DISPEL_SLOT_DEFS[i]
        local r, g, b = DispelColor(options, def)
        local key = prefix .. ':' .. def.key
        AK.styles[key] = {
            width = 1, height = 1, noRegions = true,
...
-- legal in combat (confirmed by field testing), which retired the whole
...
buildWorker:SetScript("OnUpdate", function(self)
    local profiler = _G.KT and _G.KT.CombatProfiler
    local profileStarted = profiler and profiler:Begin("core.aurakit.build")
    ...
    while buildHead <= buildTail do
        ...
        if entry then
            ...
            elseif verdict == "watchdog" then
                buildHead = buildHead - 1
                buildQueue[buildHead] = entry
                if profileStarted then profiler:End("core.aurakit.build", profileStarted) end
                return
            end
        end
...
-- scans when restriction engages within the frame window (field-hit: test-CVar flips and
