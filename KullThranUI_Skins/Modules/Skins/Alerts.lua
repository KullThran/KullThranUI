...
-- AlertFrames run inside Blizzard's protected animation/queue code.  Executing
-- KullThranUI code synchronously in AlertSystem:setUpFunction() shares that
-- protected context: any cosmetic write dirties UIParent's layout path and
-- taints a later Blizzard ActionButton update (ADDON_ACTION_BLOCKED on
-- MultiBar SetAttribute / "SetCooldown secret values" errors).  Defer every
-- skin to a clean frame context, same as Popups/GameMenu/BattleNet alerts.
local function DeferAlertSkin(frame, func)
...
local function SkinAlerts()
	...
	-- Achievements
	if _G.AchievementAlertSystem then hooksecurefunc(_G.AchievementAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinAchievementAlert) end) end
	if _G.CriteriaAlertSystem then hooksecurefunc(_G.CriteriaAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinCriteriaAlert) end) end
	if _G.MonthlyActivityAlertSystem then hooksecurefunc(_G.MonthlyActivityAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinCriteriaAlert) end) end

	-- Encounters
	if _G.DungeonCompletionAlertSystem then hooksecurefunc(_G.DungeonCompletionAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinDungeonCompletionAlert) end) end
	if _G.GuildChallengeAlertSystem then hooksecurefunc(_G.GuildChallengeAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinGuildChallengeAlert) end) end

	-- Honor
	if _G.HonorAwardedAlertSystem then hooksecurefunc(_G.HonorAwardedAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinHonorAwardedAlert) end) end

	-- Loot
	if _G.LootAlertSystem then hooksecurefunc(_G.LootAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinLootWonAlert) end) end
	if _G.MoneyWonAlertSystem then hooksecurefunc(_G.MoneyWonAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinMoneyWonAlert) end) end

	-- Pets/Mounts/Toys
	if _G.NewPetAlertSystem then hooksecurefunc(_G.NewPetAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinMiscAlerts) end) end
	if _G.NewMountAlertSystem then hooksecurefunc(_G.NewMountAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinMiscAlerts) end) end
	if _G.NewToyAlertSystem then hooksecurefunc(_G.NewToyAlertSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinMiscAlerts) end) end
	if _G.NewCosmeticAlertFrameSystem then hooksecurefunc(_G.NewCosmeticAlertFrameSystem, "setUpFunction", function(frame) DeferAlertSkin(frame, SkinMiscAlerts) end) end
