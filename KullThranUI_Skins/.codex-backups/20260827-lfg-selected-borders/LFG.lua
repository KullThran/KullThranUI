...
local function EnsureWindowAccentBorder(frame)
    ...
    for _, edge in ipairs(frame.KT_AccentEdges) do
    ...
    for _, region in ipairs({ frame:GetRegions() }) do
        if region and region.IsObjectType and region:IsObjectType("Texture") then
            ...
            if hasArtwork and (width or 0) >= 88 and (height or 0) >= 36 then
                DarkenNativeArtwork(region, 0.72, true)
            end
        end
    end
...
local function RefreshLFGNativeArtwork()
    DarkenNativeArtwork(_G.PVEFrameBg, 0.58)
    DarkenNativeArtwork(_G.LFDQueueFrameBackground, 0.72)
    ...
    DarkenNativeArtwork(_G.HonorFrame and _G.HonorFrame.WorldBattlesTexture, 0.72)
    DarkenNativeArtwork(_G.ConquestFrame and _G.ConquestFrame.RatedBGTexture, 0.72)
end
local function SkinLFG()
    ...
    if _G.LFGDungeonReadyStatus then
        S:StripTextures(_G.LFGDungeonReadyStatus)
        S:CreateBackdrop(_G.LFGDungeonReadyStatus, true)
    end

    S:HandleButton(_G.LFGDungeonReadyDialogCloseButton)
